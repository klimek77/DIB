...
- [x] 3.1 Type checking passes: `npm run typecheck` — bbebd94
- [x] 3.2 Linting passes: `npm run lint` — bbebd94
- [x] 3.3 Build succeeds: `npm run build` — bbebd94
- [x] 3.4 Redelivered already-`done` message → no AI call, ack (idempotency unit test) — bbebd94
- [x] 3.5 Transient → retry, permanent/cap → failed + signal (branching unit test) — bbebd94

#### Manual

- [x] 3.6 Happy path: `pending` → enqueue → `done` with valid tone/classification — bbebd94
- [x] 3.7 Forced-failure: row → `failed` + `enrichment_last_error` + `enrichment_failed` log event — bbebd94
- [x] 3.8 Redelivery: one AI call, no clobber — bbebd94
- [x] 3.9 Stale-`processing` reclaim works past threshold — bbebd94
- [x] 3.10 DLQ backstop: exhausted-retries message yields a `failed` row — bbebd94
