...
## Re-seed admin_allowlist after every db:reset — seed-admins reads .env, but this repo's secrets live in .dev.vars

**Context:** Local Supabase dev after `npm run db:reset`, where the RLS gate `is_allowed_admin()` reads the `public.admin_allowlist` TABLE while the app-side allow-list is the `ALLOWED_ADMIN_EMAILS` ENV. First seen: `admin-submission-triage` Phase 5 manual smoke (2026-06-23).
**Problem:** `db:reset` rebuilds the DB and migration `20260605000000` creates `admin_allowlist` EMPTY. `npm run db:seed-admins` (the script that mirrors `ALLOWED_ADMIN_EMAILS` → the table) loads env via `--env-file-if-exists=.env`, but this project keeps `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` / `ALLOWED_ADMIN_EMAILS` in `.dev.vars` (Cloudflare secrets), NOT `.env` — so the seed reads nothing and the table stays empty. The admin still LOGS IN fine (middleware + callback gate on the env-based app allow-list, which IS set), but every RLS-gated read then returns 0 rows: the dashboard renders empty at ANY range and the detail page 404s with "nie ma danych". It looks exactly like a data/feature bug; the real cause is the empty allow-list table. Cost: a long debug pass chasing the dashboard list + detail RLS before checking the table itself.
**Rule:** After every `npm run db:reset`, re-seed the DB allow-list AND verify it before trusting any "admin sees no data" symptom. `npm run db:seed-admins` alone is a no-op in this repo because the secrets live in `.dev.vars`, not `.env` — either add `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`ALLOWED_ADMIN_EMAILS` to `.env`, or run `node scripts/seed-admins.mjs` with those three vars exported from `.dev.vars`. Confirm with `SELECT email FROM public.admin_allowlist;` (must be non-empty). Empty table = fail-closed RLS: the admin authenticates but sees zero rows everywhere. Sibling to the "REVOKE ... FROM PUBLIC is a no-op" and "Push Supabase migrations to prod" lessons (same shape: app layer green, DB authz layer silently wrong).
**Applies to:** `implement`, `impl-review`
