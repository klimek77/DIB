...
- [x] 2.1 Build succeeds: `npm run build` — 21a708e
- [x] 2.2 Type checking passes: `npm run typecheck` — 21a708e
- [x] 2.3 Linting passes: `npm run lint` — 21a708e
- [x] 2.4 Full test suite green (no regression): `npm test` — 21a708e

#### Manual

- [x] 2.5 `dist/server/wrangler.json` contains `triggers.crons: ["*/15 * * * *"]` after build — 21a708e
- [x] 2.6 `wrangler dev --test-scheduled` runs the sweep; id-less summary log appears (no id/body) — 21a708e
- [x] 2.7 Seeded stranded `pending` row transitions `pending → processing → done` — 21a708e
- [x] 2.8 Re-run against a `done` row is an ack-and-skip (no second AI call) — 21a708e
